...
"""app.py
Pipeline:
1. Analisi FEM della facciata 2D (modello OpenSees ndm=2, ndf=3).
...
st.set_page_config(page_title="VoroGrid – Analisi 2D + 3D", layout="wide")

st.title("VoroGrid: analisi FEM 2D della facciata + costruzione 3D")
st.caption(
    "Modello OpenSees **ndm=2, ndf=3** (telaio piano: UX, UY, RZ). "
    "I nodi di base sono incastrati; la geometria 3D viene costruita replicando la facciata sulle quattro facce. "
...
with st.sidebar:
    ...
    face_load_case = st.selectbox("Caso di carico", ["combined", "lateral", "gravity"], index=0)

    run_btn = st.button("Genera + Analisi 2D + Costruzione 3D", type="primary")

# ── Session state ─────────────────────────────────────────────────────────────
for key in ('geometry', 'face_analysis', 'tower_geometry'):
...
if run_btn:
    ...
    with st.spinner("Generazione maglia Voronoi, analisi FEM 2D e costruzione 3D..."):
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["Facciata 2D", "Analisi FEM 2D", "Profilo deriva", "Costruzione 3D", "Esporta"]
)
...
with tab1:
    ...
    st.caption(
        ...
        f"interpiano {story_h:.2f} m — "
        f"schema densità: **{mode}**"
    )

# ── Tab 2 – Analisi FEM ────────────────────────────────────────────────────────
with tab2:
    st.subheader("Analisi FEM della facciata 2D (OpenSees ndm=2)")
    ...
    if face_analysis is None:
        ...
        h_limit  = total_height / 500.0

        m1, m2, m3, m4, m5 = st.columns(5)
        m1.metric("Caso di carico",   str(face_analysis.get('load_case', 'combined')))
        m2.metric("ux,max [m]",       f"{top_ux:.4e}")
        m3.metric("|u|max [m]",       f"{top_umag:.4e}")
        m4.metric("Drift max [−]",    f"{max_drift:.4e}")
        m5.metric("H/500 [m]",        f"{h_limit:.4e}",
                  delta=f"{'OK' if top_ux <= h_limit else 'SUPERA'} limite",
        ...
        disp_component = copt1.selectbox("Mappa spostamenti",      ["ux", "uz", "umag"], index=0)
        force_quantity = copt2.selectbox("Mappa forze di elemento", ["N", "V", "M"],     index=2)
        deformed_force = copt3.checkbox("Mostra forze sulla deformata", value=False)
        scale = st.slider("Fattore scala deformata", min_value=1.0, max_value=500.0, value=25.0, step=1.0)
        ...
        st.plotly_chart(
            plotly_face_force_map(face_analysis, quantity=force_quantity, deformed=deformed_force, scale=scale),
            use_container_width=True
        )

        periods = face_analysis.get('periods_s', [])
        if periods:
            st.subheader("Modi propri (analisi modale 2D)")
            df_eig = pd.DataFrame({
                "Modo": list(range(1, len(periods) + 1)),
                "Periodo [s]": [f"{p:.4f}" for p in periods],
                "Freq. [Hz]": [f"{1.0/p:.4f}" if p > 0 else "–" for p in periods],
            })
            st.dataframe(df_eig, use_container_width=True)
        ...
        st.info(
            "Assicurati di avere `openseespy` installato (`pip install openseespy`). "
            "Il modello usa **ndm=2, ndf=3** (telaio piano puro) — "
            "elimina i modi di corpo rigido fuori piano del precedente approccio 3D vincolato."
        )

# ── Tab 3 – Profilo deriva ─────────────────────────────────────────────────────
with tab3:
    st.subheader("Profilo spostamenti e drift inter-piano")
    if face_analysis and face_analysis.get('analysis_ok', False):
        params_dict = geometry.get('params', {})
        sh = float(params_dict.get('total_height', total_height)) / int(params_dict.get('n_stories', n_stories))
        st.plotly_chart(
            plotly_face_drift_profile(face_analysis, n_stories=int(n_stories), story_height=sh),
            use_container_width=True
        )
        st.caption("La linea tratteggiata arancione indica il limite di drift H/500 per interpiano.")
    else:
        st.warning("Esegui prima l'analisi FEM nel tab **Analisi FEM 2D**.")

# ── Tab 4 – Costruzione 3D ────────────────────────────────────────────────────
with tab4:
    st.subheader("Costruzione geometrica 3D dalla facciata 2D")
    if tower_geometry is not None:
        st.plotly_chart(plotly_tower_traces(tower_geometry), use_container_width=True)
        st.caption(
            "La facciata 2D viene replicata sulle quattro facce della torre "
            "e si aggiunge il nucleo rettangolare equivalente. Analisi FEM 3D non inclusa in questa fase."
        )
